...
def gradient_descent(X, Y, theta, alpha, iteraciones):
    ...
    for iter in range(iteraciones):
        ...
        loss = (y_hat - Y)**2
        # loss = (Y - y_hat)**2     Revisar si es y - y_hat o y_hat - y

        gradient = X.T.dot(y_hat - Y) * 2/X.shape[0]
        # gradient = X.T.dot(Y - y_hat) * 2/X.shape[0]  Revisar si es y - y_hat o y_hat - y
